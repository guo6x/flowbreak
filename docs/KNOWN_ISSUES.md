...
> 
> - **Bug ID 唯一性**：一个 ID 在整个项目生命周期只能代表一个缺陷，禁止复用。历史已解决项使用 HIST-xxx 编号或不分配 ID。
> - **证据存储**：设备原始证据不入 Git 仓库，由测试工作区外部保存；下文 Evidence 中的 External device evidence: 路径为测试工作区报告名。
> - **状态语义**：master 已含修复提交但未完成 Redmi 复测时，状态保持 OPEN，并标注 IMPLEMENTED / awaiting device revalidation。
...
- Severity：P1
- Status：**OPEN** — IMPLEMENTED（master `027af94`）/ awaiting device revalidation R1（2026-08-12 第四次复现）
...
- Evidence：External device evidence: `reports/BUG-P1-foreground-tracker.md`（含 usagestats 事件序列与 sessionMs 采样）
...
- Severity：P1
- Status：**OPEN** — IMPLEMENTED（master `9c34fe9`）/ awaiting device revalidation R2（2026-08-12 实测）
- Affected version/SHA：`a06a772bd0b12bc6a31e78d91a4d634ec7027437`
...
- Evidence：External device evidence: `reports/T38-strong-blocking.md` §6
...
- Severity：P1 候选（兼容性）
- Status：**OPEN** — IMPLEMENTED（master `3600d97`）/ awaiting device revalidation R3（2026-08-12 两次实测被拒）
- Affected version/SHA：`a06a772bd0b12bc6a31e78d91a4d634ec7027437`
- Environment：Redmi Note 13 Pro 5G / Android 16 / HyperOS 3.0 / domestic debug APK
...
- Evidence：External device evidence: `reports/T38-strong-blocking.md` §3
...
- Severity：P2
- Status：**OPEN** — IMPLEMENTED（master `99fdcc2`）/ awaiting device revalidation R4（2026-08-12 实测）
- Affected version/SHA：`a06a772bd0b12bc6a31e78d91a4d634ec7027437`
- Environment：Redmi Note 13 Pro 5G / Android 16 / HyperOS 3.0 / domestic debug APK
- Observed：目标应用选择页存在未保存修改时，页面内返回按钮正常弹「有未保存的修改」对话框；系统返回键/手势返回直接退出到桌面，修改静默丢失。
- Expected：所有退出路径（含系统返回键）行为一致，未保存修改需确认。
- Evidence：External device evidence: `reports/BUG-P2-back-key.md`
...
- **HIST-001 前台识别改用 UsageEvents**（历史已解决项，不占用 Bug ID）：原「`queryUsageStats` lastTimeUsed 不可靠」问题已通过 UsageEvents 重构（`ForegroundUsageDetector`/`ForegroundAppTracker`）解决。当前 `FB-P1-01`（同包 Activity 切换误清前台）是独立缺陷，仍 OPEN。
