...
- **安全收尾（Stage B1，2026-08-16）**：首轮密钥（Play `ebbf4e21…e095` / Domestic `bbb2ead0…5f0e`）因初始密码经过自动化进程与 `OWNER-PASSWORD-HANDOFF.txt`，降级 **SUPERSEDED_PRE_PRODUCTION**（无泄露证据、非安全事故、禁用于正式发行）。工作流已收紧：release job 条件 = `workflow_dispatch && refs/heads/master` 或 `v*` tag + `SIGNED_DISPATCH_REF=PASS` 显式守卫（PR #6）；负向测试要求：非 master dispatch 时 release job 不得进入 production-signing（`NON_MASTER_SIGNING_ACCESS = DENIED`）。Environment 部署分支策略（master + v*）API 对公共仓库 404，由产品负责人 UI 设置后经 API 读取确认。最终身份 = `HUMAN_KEY_GENERATION_REQUIRED`：产品负责人独立终端以 keytool 交互式密码生成最终两把密钥（命令不含 -storepass/-keypass），AI 不生成/读取/保存/接收密码，禁止再创建 password handoff 文件；AI 仅从产品负责人导出的 **public certificate** 独立计算 fingerprint 并与其 keytool 输出交叉核对。
