...
public class FlowDatabaseMigrationTest {
    ...
    private final MigrationTestHelper helper;
        ...
        try (Cursor c = db.query("PRAGMA table_info(daily_summary)")) {
            ...
            while (c.moveToNext()) {
                ...
                if (n.equals("successfulPullbackCount") || n.equals("pullbackOutcomeCount")
                        ...
                        || n.equals("reflectionValue") || n.equals("reflectionUpdatedAt")) hasV3Field = true;
            }
    ...
    public void migrate2To3_preservesSummaryAndAddsDefaults() throws Exception {
        ...
        try (Cursor c = db.query("PRAGMA table_info(daily_summary)")) {
            while (c.moveToNext()) {
                String n = c.getString(c.getColumnIndexOrThrow("name"));
                if (n.equals("successfulPullbackCount") || n.equals("pullbackOutcomeCount") || n.equals("postRestReturnCount") || n.equals("reflectionValue") || n.equals("postRestTargetSeconds") || n.equals("reflectionUpdatedAt")) {
    ...
    public void migrate1To3_preservesAllDataAndEnablesDao() throws Exception {
        ...
        try (Cursor c = db.query("PRAGMA table_info(daily_summary)")) {
            int fc = 0;
            while (c.moveToNext()) fc++;
            assertEquals(12, fc);
        }
        try (Cursor c = db.query("SELECT name FROM sqlite_master WHERE type='table' AND name NOT LIKE 'android_%' AND name NOT LIKE 'room_%'")) {
            int tc = 0;
            while (c.moveToNext()) { tc++; }
            assertEquals(4, tc);
        }
        db.close();
