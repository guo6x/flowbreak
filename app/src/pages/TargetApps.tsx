...
export default function TargetApps() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      if (!Capacitor.isNativePlatform()) {
        setApps(webApps);
        setSelected(current => current.length ? current : DEFAULT_TARGET_APPS);
        return;
      }
      ...
      setChannel(settings.channel === 'play' || settings.channel === 'domestic' ? settings.channel : 'base');
  ...
  const toggle = (packageName: string) => {
    setError('');
    setSelected(current => {
      if (current.includes(packageName)) return current.filter(pkg => pkg !== packageName);
    ...
    <div className="min-h-dvh px-5 pt-6 pb-28">
      ...
      {channel === 'play' && (
        <div className="mb-4 p-4 bg-amber-50 border border-amber-200 rounded-2xl text-[12px] text-amber-900 leading-relaxed">
          Google Play 版不会把整个微信当作“视频号”阻断：系统无法可靠区分聊天和视频号。请在这里选择抖音、小红书、快手等可明确识别的应用。
        </div>
      )}
      {channel === 'domestic' && (
      ...
      {loading && <p className="text-center text-gray-500 py-12">正在读取应用...</p>}
      {!loading && apps.length === 0 && (
        <button onClick={load} className="w-full py-4 rounded-2xl bg-gray-100 text-gray-700 flex items-center justify-center gap-2">
          <RefreshCw size={17} />
          重新读取应用列表
        </button>
      )}
        ...
        <div className="card overflow-hidden">
          {filtered.map(app => {
            ...
            return (
              ...
              >
                {app.iconDataUri ? (
                  <img src={app.iconDataUri} alt="" className="w-10 h-10 rounded-xl" />
                ) : (
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-[14px] font-medium truncate">{app.label}</p>
                  <p className="text-[10px] text-gray-400 truncate">{app.packageName}</p>
                </div>
                <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center ${active ? 'bg-primary border-primary' : 'border-gray-300'}`}>
